...
public class SonosService implements SonosSoap {
    ...
    public SonosService() {
        AUTH_API_URI = System.getenv("AUTH_API_URI");
        GRAPH_API_URI = System.getenv("GRAPH_API_URI");
    ...
    throws CustomFault {
        ...
        if (parameters.getId().equals("root")) {
            String path = "/me/drive/root:/Music:/children";
            ...
            ml = parseMediaListResponse(auth.getHouseholdId(), json, 0);
            ...
            String path_count = String.format("/me/drive/items/%s", parameters.getId().replaceAll(SonosService.FOLDER + ":", ""));
            String skipToken = null;
            int childCount = 0;
            ...
            String json = graphApiGetRequest(path, parameters.getCount(), skipToken, auth);
            String json_count = graphApiGetRequest(path_count, 1, null, auth);

            JsonElement element = JsonParser.parseString(json_count);
            if (element.isJsonObject()) {
                JsonObject root = element.getAsJsonObject();
                JsonObject folderAttributes = root.getAsJsonObject("folder");
                childCount = folderAttributes.has("childCount") ? folderAttributes.get("childCount").getAsInt() : 0;
                logger.debug("Setting childCount to " + childCount);
            }

            ml = parseMediaListResponse(auth.getHouseholdId(), json, childCount);
    ...
    private static MediaList parseMediaListResponse(String userId, String json, int childCount) {
        JsonElement element = JsonParser.parseString(json);
    ...
    public SearchResponse search(Search parameters) throws CustomFault {
        ...
        logger.info("Sonos search parameters - Index:" + parameters.getIndex() + " Count:" + parameters.getCount() + " Search-Term: " + parameters.getTerm());

        // onedrive business search returns a maximum of 25 search results per request, total count and top/skip is not working
        JsonArray jsonArray = new JsonArray();
        JsonArray jsonArray1 = new JsonArray();
        boolean isSearchComplete = false;
        int maxSearchResults = parameters.getIndex() + parameters.getCount();
        for (int j = 0; j < maxSearchResults; j += 25) {
            logger.debug("in for loop: " + j);
            String json = graphApiGetRequest(path, 25, skipToken, auth);
            JsonElement element = JsonParser.parseString(json);
            jsonArray1 = element.getAsJsonObject().getAsJsonArray("value");

            for (int i = 0; i < jsonArray1.size(); i++) {
                Item m = new Item(jsonArray1.get(i).getAsJsonObject());
                if (m.getType().equals(Item.FileType.audio) || m.getType().equals(Item.FileType.folder)) {
            ...
            if (element.getAsJsonObject().has("@odata.nextLink")) {
                Pattern pattern = Pattern.compile("\\$skiptoken=(.+)", Pattern.CASE_INSENSITIVE);
                Matcher matcher = pattern.matcher(element.getAsJsonObject().get("@odata.nextLink").getAsString());
                if (matcher.find()) {
                    skipToken = matcher.group(1);
                    logger.debug("skiptoken found ..");
                } else {
                    logger.debug("no skiptoken found, breaking ..");
                    isSearchComplete = true;
                    break;
                }
            } else {
                logger.debug("no nextlink found, breaking ..");
                isSearchComplete = true;
                break;
            }
        }
        ...
        if (!isSearchComplete) {
            for (int k = jsonArray.size() - 1; k >= maxSearchResults; k--) {
                jsonArray.remove(k);
            }
        }

        //if (parameters.getIndex() > 0) {
        //    for (int l = parameters.getIndex() - 1; l >= 0; l--) {
        //        jsonArray.remove(l);
        //    }
        //}
        ...
        MediaList ml = new MediaList();
        ml = parseMediaListResponse(auth.getHouseholdId(), "{\"value\":" + jsonArray.toString() + "}", 0);
        ml.setIndex(parameters.getIndex());
        ...
        logger.info("Search result Index:" + ml.getIndex() + " Count:" + ml.getCount() + " Total: " + ml.getTotal());
        return response;
    }
